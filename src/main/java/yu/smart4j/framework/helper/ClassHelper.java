...
public final class ClassHelper {
    ...
    /**
     * 获取应用包名下的所有bean类（Controller、Service合集）
     */
     ...
     * 获取应用包名下带有某注解的所有类
     */
    public static Set<Class<?>> getClassSetByAnnotation(Class<? extends Annotation> annotationsClass) {
        Set<Class<?>> classSet = new HashSet<Class<?>>();
        for (Class<?> cls : CLASS_SET) {
            if (cls.isAnnotationPresent(annotationsClass)) {
                classSet.add(cls);
            }
        }
        return classSet;
